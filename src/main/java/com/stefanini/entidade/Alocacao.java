...
@NamedQueries({
	...
	@NamedQuery(name = "Alocacao.findRecursoParaDesativar", query = "SELECT a FROM Alocacao a WHERE a.recurso.profissional.matricula = :matricula AND a.projeto.codigo = :codigo AND(a.registroValidadeFim IS NULL OR a.registroValidadeFim > CURRENT_DATE) AND a.registroValidadeInicio <= CURRENT_DATE ORDER BY a.recurso.profissional.nome ASC"),
