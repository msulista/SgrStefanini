...
@NamedQueries({
	...
	@NamedQuery(name = "Profissional.findProfissionalByCelulaANDEquipeEPerfil", query ="SELECT p FROM Profissional p WHERE p.equipe.nome = :nome AND p.perfil.nome = :serie AND p.celula.id =:id AND p.registroValidadeInicio <= CURRENT_DATE AND (p.registroValidaeFim IS NULL OR p.registroValidaeFim > CURRENT_DATE) ORDER BY p.nome ASC)"),
	@NamedQuery(name = "Profissional.findProfissionalByCelulaEPerfil", query ="SELECT p FROM Profissional p WHERE p.celula.id = :celula AND p.perfil.nome = :serie AND p.registroValidadeInicio <= CURRENT_DATE AND (p.registroValidaeFim IS NULL OR p.registroValidaeFim > CURRENT_DATE) ORDER BY p.nome ASC)")
